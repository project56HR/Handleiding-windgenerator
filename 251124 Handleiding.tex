...
\begin{document}
	...
	De windgenerator wordt aangesloten aan de aansturingskast. Hier zitten vier verschillende poorten op. De stekker die van de windgenerator af komt past maar op een van deze poorten, namelijk de poort gelabeld \textbf{“GEN IN”}.  zie afbeelding (a) hieronder
\begin{figure}[h]
\begin{subfigure}{0.5\linewidth}
\includegraphics[width=1.2\linewidth, height=8cm]{.jpg} 
\caption{aansturings kast}
\label{fig:aansturings kast}
\end{subfigure}
\begin{subfigure}{0.5\linewidth}
\centering
\includegraphics[width=0.5\linewidth, height=8cm]{systeemShakelaar.jpg}
\caption{systeem schakelaar}
\label{fig:systeem schakelaar}
\end{subfigure}
\end{figure}

    
    Schakel vervolgens het systeem in. Dit kan worden gedaan via de schakelaar \textbf{“SYSTEM”} worden gedaan. Zodra deze omgehaald is zal het scherm op de aansturingskast opstarten en zullen alle interne systemen opstarten. zie afbeelding (b) hierboven
    \\
    \\
...
\begin{figure}
    \centering
    \includegraphics[width=0.25\linewidth]{schakelaar.jpg}
...
\end{figure}
    ...
    \section{MQTT}
    \subsection{automatisch opstarten MQTT}

    \begin{itemize}
        \item Stap 1: voer dit in bij de terminal
        \begin{lstlisting}
			sudo nano /ect/systemd/system/chromium-startup.service
		\end{lstlisting}
            \item Stap 2: kopieer (CTRL c) de onderstaande tekst en plak (CTRL v) deze in de terminal.
        \begin{lstlisting}
			[Unit]
            Description=Start Chromium after graphical session
            ...
            ExecStart=/usr/bin/chromium --incognito --noerrdialogs http://192.168.1.101
            Restart=on-failure

            [Install]
            WantedBy=graphical.target
		\end{lstlisting}
        Type vervolgends CTRL x in op het op te slaan. en druk dan op ENter
        \item Stap 3: Type vervolgends:
        \begin{lstlisting}
			sudo systemct1 enable chromium-startup.service
		\end{lstlisting}
        Druk daarna op ENTER
        \item Stap 4: Type dan:
        \begin{lstlisting}
			sudo systemct1 start chromium-startup.service
		\end{lstlisting}
        en druk vervolgends op ENTER.
    \end{itemize}
	...
	\begin{itemize}
		...
		\end{lstlisting}
		 ...
		 \item Stap 7: Controleer of alles draait. Dit doe je doormiddel van:
		 \begin{lstlisting}[language=Bash, caption={Checking if the container was build succesfully}]
		 	docker ps
		 \end{lstlisting} 
		 Als alles draait zie je de volgende containers staan:
		 \begin{itemize}
		 	\item \textit{MobileGenerator}
		 	\item \textit{mysql\_db}
		 \end{itemize}
		 
		 	\item Stap 8: USB-Stick veilig verwijderen (belangrijk!)
		 	\\
	...
	\newpage
	\section{Tailscale}
	\subsection{Installatie van Tailscale op de Raspberry Pi}
	
	\begin{itemize}
		\item Stap 0: Open de terminal aan de hand van de instructies in hoofdstuk 3.2.1 (Raspberry Pi terminal).
		
		\item Stap 1: Zorg ervoor dat het systeem up-to-date is door het volgende in te voeren:
		\begin{lstlisting}[language=Bash]
			sudo apt update && sudo apt upgrade -y
		\end{lstlisting}
		
		\item Stap 2: Download en installeer Tailscale via het officiële installatie-script:
		\begin{lstlisting}[language=Bash]
			curl -fsSL https://tailscale.com/install.sh | sh
		\end{lstlisting}
		Dit script installeert Tailscale automatisch voor jouw besturingssysteem (Raspberry Pi OS).
		
		\item Stap 3: Controleer of Tailscale correct is geïnstalleerd:
		\begin{lstlisting}[language=Bash]
			tailscale version
		\end{lstlisting}
		Als alles goed is gegaan, zie je nu de versie van Tailscale.
	\end{itemize}
	
	\subsection{Tailscale verbinden met een account}
	
	\begin{itemize}
		\item Stap 1: Start Tailscale met het volgende commando:
		\begin{lstlisting}[language=Bash]
			sudo tailscale up
		\end{lstlisting}
		
		\item Stap 2: Na het uitvoeren van dit commando verschijnt er een URL in de terminal, bijvoorbeeld:
		\begin{lstlisting}
			https://login.tailscale.com/a/abcdef123456
		\end{lstlisting}
		
		\item Stap 3: Kopieer deze link en open deze in een webbrowser op je laptop of PC.
		
		\item Stap 4: Log in met je Tailscale-account (bijvoorbeeld via Google, Microsoft of GitHub).
		
		\item Stap 5: Na het inloggen wordt de Raspberry Pi automatisch toegevoegd aan je Tailscale-netwerk.
	\end{itemize}
	
	\subsection{Controleren of Tailscale werkt}
	
	\begin{itemize}
		\item Stap 1: Controleer de status van Tailscale met:
		\begin{lstlisting}[language=Bash]
			tailscale status
		\end{lstlisting}
		
		\item Stap 2: Je ziet nu een lijst met apparaten die verbonden zijn met jouw Tailscale-netwerk.  
		Hier zou ook de Raspberry Pi tussen moeten staan.
		
		\item Stap 3: Noteer het Tailscale-IP-adres van de Raspberry Pi (bijvoorbeeld \texttt{100.x.x.x}).  
		Dit IP-adres kan worden gebruikt om de Raspberry Pi veilig op afstand te bereiken.
	\end{itemize}
	
	\subsection{Automatisch opstarten van Tailscale}
	
	\begin{itemize}
		\item Stap 1: Controleer of de Tailscale service automatisch start bij het opstarten:
		\begin{lstlisting}[language=Bash]
			sudo systemctl status tailscaled
		\end{lstlisting}
		
		\item Stap 2: Als de service nog niet actief is, start deze handmatig:
		\begin{lstlisting}[language=Bash]
			sudo systemctl enable tailscaled
			sudo systemctl start tailscaled
		\end{lstlisting}
		
		\item Stap 3: Tailscale zal nu automatisch starten bij elke reboot van de Raspberry Pi.
	\end{itemize}
